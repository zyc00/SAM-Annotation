...
class ImageViewer:
    def __init__(
        ...
        self.graphicsView.setScene(
            self.scene
        )  # Set the scene of the passed graphicsView
        ...
        self.graphicsView.mousePressEvent = (
            self.graphicsViewMousePressEvent
        )  # Override the mousePressEvent for graphicsView
        self.annList.itemClicked.connect(self.highlightMask)

        # Load SAM model
        sam_checkpoint = config["checkpoint_path"]
    ...
    def openImageFolder(self, folder_path=None):
    ...
    def graphicsViewMousePressEvent(self, event):
        ...
        if self.pixmapItem and self.pixmapItem.contains(position):
            ...
            dot = QGraphicsEllipseItem(
                relative_position.x(), relative_position.y(), 10, 10
            )
            ...
            mask, score, logit = self.predictor.predict(
                point_coords=np.array(
                    [point_info["position"] for point_info in self.pointStack]
                ).astype(float),
                point_labels=np.array(
                    [point_info["label_type"] for point_info in self.pointStack]
                ).astype(float),
                multimask_output=self.logits is None,
                mask_input=self.logits,
            )
            mask = mask[np.argmax(score)]
            self.logits = logit[np.argmax(score)][None, ...]
            self.pointStack[-1]["mask"] = mask
            self.displayMask(mask)
            # Optionally, you might want to store or use the click position
            print(
                f"Clicked position relative to image: ({relative_position.x()}, {relative_position.y()})"
            )
    ...
    def getMaskContourPoints(self, mask):
        ...
        assert (
            np.setdiff1d(binary_mask, [0, 255]).size == 0
        ), "binary_mask must contain only 0 and 255"
    ...
    def finishAnnotation(self):
        if self.pointStack:
            ...
            mask_contour_points = self.getMaskContourPoints(
                last_annotation_info["mask"]
            )
